...
class Sample:
    ...
    def __init__(self):
        ...
        Parameters:
            sites (list): It receives a list with the index values, and the 
            positions of the sites in the system.
            neighbors (list): It receives the list of neighbors of the sites in 
            the system.
            units (str): It receives the units of the Boltzmann constant.
            damping (float): It receives the damping constant of the sites in the 
            system.
            ...
            temperature (float/list/dict): It receives the temperature information 
            of the sites in the system.
            field (float/list/dict): It receives the field information of the 
            sites in the system.
            jex (float): It receives the exchange interaction of the sites in 
            the system.
            mu (float): It receives the spin norms of the sites in the system.
            field_axis (list): It receives the field axis of the sites in the 
            system.
            type (str): It receives the type of the sites in the system.
            anisotropy_constant (float): It receives the anisotropy constants of 
            the sites in the system.
            anisotopy_axis (list): It receives the anisotropy axis of the sites 
            in the system.
            seed (int): It receives the number of the seed, this is because we 
            want to generate the same number every time before calling ``random.randint()``.
            initial_state (list): It receives the initial state of the sites in 
            the system.
        """
        self.sites = []
        self.neighbors = []
    ...
    def build(self):
        """
        It is a function responsible for building the sample. It receives all
        the attributes of the Sample class. This function ensures that all 
        attributes were entered. If any of them is missing, the function throw 
        an ``Exception``.

        Returns:
            sample (dict): It is a dictionary with all the attributes organized.
        """
